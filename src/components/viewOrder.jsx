...
async function buildProductMap(productIds) {
  ...
  try {
    ...
    while (pageNum <= totalPages && needed.size > 0) {
      const res = await axios.get(`http://localhost:8080/products?pageNum=${pageNum}`, { headers });
      const products = res.data?.content || (Array.isArray(res.data) ? res.data : []);
  ...
  await Promise.all(
    [...needed].map(async (id) => {
      try {
        const res = await axios.get(`http://localhost:8080/products/${id}`, { headers });
        map[id] = res.data;
      } catch {
        /* ignore */
      }
  ...
  useEffect(() => {
    const loadOrders = async () => {
      try {
        ...
        const orderList = normalizeOrders(Array.isArray(data) ? data : []);
        ...
        setOrders(enrichOrdersWithProducts(orderList, productMap));
        setError(null);
      } catch (err) {
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${getStatusStyle(order.status)}`}
                        >
                  ...
                  {isExpanded && (
                    <div className="border-t border-gray-200 px-6 pb-6">
                      {order.items && order.items.length > 0 ? (
                        ...
                        <p className="py-4 text-sm text-gray-500">No items found for this order.</p>
                      )}
